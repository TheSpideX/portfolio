  ...
  return (
    ...
    >
      <div ref={floatRef} className={`w-full h-full node-element ${innerClassName}`}>
        <div ref={tiltRef} className="w-full h-full relative flex flex-col" style={{ transformStyle: 'preserve-3d' }}>
          ...
          {(() => {
            ...
            return (
              <div className={`w-full flex-1 min-h-0 flex flex-col transition-opacity duration-700 ${isSkeleton ? 'opacity-0 pointer-events-none' : 'opacity-100'}`} style={{ transformStyle: 'preserve-3d' }}>
                {children}
              </div>
            );
          })()}
